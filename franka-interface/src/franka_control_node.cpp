...
#include "utils/traj_interpolators/ruckig_pose_traj_interpolator.h"
...
bool GetTrajInterpolatorType(const FrankaControlMessage &franka_control_msg,
  ...
  if (franka_control_msg.traj_interpolator_type() ==
    ...
    traj_interpolator_type = TrajInterpolatorType::RUCKIG_POSE;
  }
  else {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    int zmq_idle_terminate_steps = 20;
    if (control_config["CONTROL"]["ZMQ_IDLE_TERMINATE_STEPS"]) {
      zmq_idle_terminate_steps =
          control_config["CONTROL"]["ZMQ_IDLE_TERMINATE_STEPS"].as<int>();
    }
    global_handler->logger->info("ZMQ idle terminate steps: {0}",
                                 zmq_idle_terminate_steps);
    ...
    std::thread control_msg_sub([&, zmq_idle_terminate_steps]() {
      ControllerType runtime_controller_type = ControllerType::NO_CONTROL;
      TrajInterpolatorType runtime_traj_interpolator_type =
          TrajInterpolatorType::NO_INTERPOLATION;
      StateEstimatorType runtime_state_estimator_type =
          StateEstimatorType::NO_ESTIMATOR;
      uint64_t zmq_last_control_session = 0;
      while (!global_handler->termination) {
        ...
        if (msg.length() == 0) {
          ...
          if (zmq_idle_terminate_steps > 0 &&
              global_handler->no_msg_counter >= zmq_idle_terminate_steps) {
            global_handler->running = false;
            global_handler->termination = true;
            global_handler->logger->debug(
                "No valid messages received in {0} steps.",
                zmq_idle_terminate_steps);
          }
          continue;
        }
        ...
        if (control_msg.ParseFromString(msg)) {
          ...
          const uint64_t incoming_control_session = control_msg.control_session();
          const bool session_gated = incoming_control_session != 0UL;
          const bool hard_session_reset = control_msg.session_hard_reset();
          const bool should_reset_traj =
              hard_session_reset ||
              (session_gated && zmq_last_control_session != 0UL &&
               incoming_control_session != zmq_last_control_session);
          if (session_gated) {
            zmq_last_control_session = incoming_control_session;
          }

          ControllerType next_controller_type = ControllerType::NO_CONTROL;
          TrajInterpolatorType next_traj_interpolator_type =
              TrajInterpolatorType::NO_INTERPOLATION;
          StateEstimatorType next_state_estimator_type =
              StateEstimatorType::NO_ESTIMATOR;

          if (!GetControllerType(control_msg, next_controller_type)) {
            global_handler->logger->warn("No controller type is specified");
          }
          if (!GetTrajInterpolatorType(control_msg, next_traj_interpolator_type)) {
            global_handler->logger->debug("No traj interpolator is specified");
          }
          if (!GetStateEstimatorType(control_msg, next_state_estimator_type)) {
            global_handler->logger->debug("No state estimator is specified");
          }
          const double next_traj_time_fraction =
              control_msg.traj_interpolator_time_fraction() > 0.10
                  ? control_msg.traj_interpolator_time_fraction()
                  : 0.1;

          {
            std::lock_guard<std::mutex> command_lock(control_command.mutex);
            control_command.controller_type = next_controller_type;
            control_command.traj_interpolator_type =
                next_traj_interpolator_type;
            control_command.state_estimator_type = next_state_estimator_type;
          ...
          if (should_reset_traj) {
            global_handler->logger->info(
                "Control session reset: session={0} hard={1} last={2}",
                incoming_control_session, hard_session_reset,
                zmq_last_control_session);
            global_handler->traj_interpolator_ptr.reset();
            global_handler->ResetRuntimeStateLocked();
            runtime_traj_interpolator_type = TrajInterpolatorType::NO_INTERPOLATION;
            runtime_state_estimator_type = StateEstimatorType::NO_ESTIMATOR;
          }

          if (next_controller_type != ControllerType::NO_CONTROL &&
              runtime_controller_type != ControllerType::NO_CONTROL &&
              next_controller_type != runtime_controller_type) {
            global_handler->logger->warn(
                "Received controller switch from {0} to {1} without NO_CONTROL handoff. "
                "Stopping current controller and waiting for the next message.",
                static_cast<int>(runtime_controller_type),
                static_cast<int>(next_controller_type));
            global_handler->running = false;
            global_handler->controller_ptr.reset();
            global_handler->traj_interpolator_ptr.reset();
            global_handler->ResetRuntimeStateLocked();
            runtime_controller_type = ControllerType::NO_CONTROL;
            runtime_traj_interpolator_type = TrajInterpolatorType::NO_INTERPOLATION;
          ...
          if (runtime_controller_type == ControllerType::NO_CONTROL ||
            ...
            runtime_controller_type = next_controller_type;
            runtime_state_estimator_type = StateEstimatorType::NO_ESTIMATOR;
            global_handler->ResetRuntimeStateLocked();
            global_handler->running = true;
          }

          if (next_state_estimator_type != runtime_state_estimator_type ||
              runtime_state_estimator_type == StateEstimatorType::NO_ESTIMATOR) {
            global_handler->controller_ptr->SetStateEstimator(
                std::make_shared<
                    estimator_utils::ExponentialSmoothingEstimator>());
            if (runtime_state_estimator_type == StateEstimatorType::NO_ESTIMATOR) {
              global_handler->logger->info("Initialize State Estimator");
            }
            runtime_state_estimator_type = next_state_estimator_type;
          }

          global_handler->start = true;
          global_handler->traj_interpolator_time_fraction =
              next_traj_time_fraction;

          if (next_traj_interpolator_type != runtime_traj_interpolator_type ||
              !global_handler->traj_interpolator_ptr) {
            if (next_traj_interpolator_type == TrajInterpolatorType::LINEAR_POSE) {
              global_handler->traj_interpolator_ptr =
                  std::make_shared<traj_utils::LinearPoseTrajInterpolator>();
              global_handler->logger->info("Initialize Pose interpolator!");
            } else if (next_traj_interpolator_type ==
              ...
              global_handler->logger->info(
                  "Initialize Linear Joint Position Trajectory Interpolator");
                       ...
                       TrajInterpolatorType::COSINE_CARTESIAN_VELOCITY) {
              global_handler->traj_interpolator_ptr = std::make_shared<
                  traj_utils::CosineCartesianVelocityTrajInterpolator>();
              global_handler->logger->info(
                  "Initialize Cosine Cartesian Velocity Trajectory Interpolator");
            } else if (next_traj_interpolator_type ==
                       TrajInterpolatorType::LINEAR_CARTESIAN_VELOCITY) {
              global_handler->traj_interpolator_ptr = std::make_shared<
                  traj_utils::LinearCartesianVelocityTrajInterpolator>();
              global_handler->logger->info(
                  "Initialize Linear Cartesian Velocity Trajectory Interpolator");
            } else if (next_traj_interpolator_type ==
                       TrajInterpolatorType::RUCKIG_POSE) {
              global_handler->traj_interpolator_ptr =
                  std::make_shared<traj_utils::RuckigPoseTrajInterpolator>();
              global_handler->logger->info(
                  "Initialize Ruckig Pose Trajectory Interpolator");
            } else {
              global_handler->traj_interpolator_ptr.reset();
              global_handler->logger->error("No interpolator is specified");
            }

            runtime_traj_interpolator_type = next_traj_interpolator_type;
          }

          if (!global_handler->controller_ptr->ParseMessage(control_msg)) {
            global_handler->logger->error(
                "Failed to parse control message for controller type {0}",
                static_cast<int>(next_controller_type));
            continue;
          }
          if (global_handler->traj_interpolator_ptr != nullptr) {
            global_handler->traj_interpolator_ptr->SetConfig(
                control_msg.traj_interpolator_config());
          }
          ...
          if (global_handler->traj_interpolator_ptr != nullptr) {
            ...
            switch (next_traj_interpolator_type) {
            case TrajInterpolatorType::LINEAR_POSE:
            case TrajInterpolatorType::LINEAR_POSITION:
            case TrajInterpolatorType::MIN_JERK_POSE:
            case TrajInterpolatorType::RUCKIG_POSE:
              global_handler->traj_interpolator_ptr->Reset(
                  global_handler->time,
                  current_state_info->pos_EE_in_base_frame,
                  current_state_info->quat_EE_in_base_frame,
                  goal_state_info->pos_EE_in_base_frame,
                  goal_state_info->quat_EE_in_base_frame, policy_rate,
            ...
            case TrajInterpolatorType::LINEAR_CARTESIAN_VELOCITY:
              global_handler->traj_interpolator_ptr->Reset(
                  ...
                  goal_state_info->twist_rot_EE_in_base_frame, policy_rate,
                  traj_rate, global_handler->traj_interpolator_time_fraction);
              break;
            default:
              break;
    ...
    // Main loop
    global_handler->logger->info("Controller starting");
    ControllerType requested_controller_type = ControllerType::NO_CONTROL;
    while (!global_handler->termination) {
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
      // If controller_type changes, exit robot control loop and reinitialize.
      {
        std::lock_guard<std::mutex> command_lock(control_command.mutex);
        requested_controller_type = control_command.controller_type;
      }

      if (global_handler->running) {
        init_state = robot.readOnce();
        state_publisher->UpdateNewState(init_state, &model);
        if (requested_controller_type == ControllerType::NO_CONTROL)
          continue;
        // Choose which control callback functions
        if (requested_controller_type == ControllerType::OSC_POSE ||
            requested_controller_type == ControllerType::OSC_POSITION ||
            requested_controller_type == ControllerType::OSC_YAW) {
        ...
        } else if (requested_controller_type == ControllerType::JOINT_IMPEDANCE) {
          ...
          robot.control(control_callbacks::CreateTorqueFromJointSpaceCallback(
              global_handler, state_publisher, model, current_state_info,
              goal_state_info, policy_rate, traj_rate));
        } else if (requested_controller_type == ControllerType::JOINT_POSITION) {
          ...
          robot.control(control_callbacks::CreateJointPositionCallback(
              global_handler, state_publisher, model, current_state_info,
              goal_state_info, policy_rate, traj_rate));
        } else if (requested_controller_type == ControllerType::CARTESIAN_VELOCITY) {
