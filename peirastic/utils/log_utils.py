...
from peirastic.utils.config_utils import config_root
from peirastic.utils.yaml_config import YamlConfig


class PeirasticDefaultLogger:
    def __init__(self, logger_config_path):
        if logger_config_path is None:
            logger_config_path = os.path.join(config_root, "peirastic_default_logger.yml")
...
class PeirasticColorFormatter(logging.Formatter):
    format_str = "[Controller %(levelname)s] "
...
class PeirasticExampleColorFormatter(logging.Formatter):
    format_str = "[Controller Examples %(levelname)s] "
...
def get_peirastic_logger(logger_config_path=None):
    """This function returns a logger that follows the peirastic convention"""
    PeirasticDefaultLogger(logger_config_path)
    logger = logging.getLogger("peirastic")
    return logger


def get_peirastic_example_logger(logger_config_path=None):
    """This function returns a logger that follows the peirastic convention"""
    PeirasticDefaultLogger(logger_config_path)
    logger = logging.getLogger("peirastic_examples")
    return logger
