...
def add_robot_config_arguments(parser):
    ...
    parser.add_argument("--folder", type=Path, default="/tmp/peirastic_demo_data")
...
def verify_controller_config(controller_cfg: dict, use_default=True):
    ...
    if controller_cfg["controller_type"] in [
        ...
        if not check_attr(controller_cfg, "traj_interpolator_cfg"):
            ...
            logger.warning("field traj_interpolator_type not specified!!!")
            field_missing = True
        ...
        if not check_attr(controller_cfg, "action_scale"):
            controller_cfg["action_scale"] = {"translation": 1.0, "rotation": 1.0}
        ...
        if not check_attr(controller_cfg, "traj_interpolator_cfg"):
            controller_cfg["traj_interpolator_cfg"] = {
                "traj_interpolator_type": "RUCKIG_POSE",
                "time_fraction": 0.5,
            }
            logger.warning("field traj_interpolator_cfg not specified!!!")
            field_missing = True
            ...
            field_missing = True
        elif not check_attr(controller_cfg["traj_interpolator_cfg"], "time_fraction"):
            controller_cfg["traj_interpolator_cfg"]["time_fraction"] = 0.5
            field_missing = True
        ...
        if uses_pose_tracking:
            if not check_attr(controller_cfg, "Kp"):
                ...
                field_missing = True
            elif not check_attr(controller_cfg["Kp"], "translation"):
                controller_cfg["Kp"]["translation"] = [3.5] * 3
                field_missing = True
            elif type(controller_cfg["Kp"]["translation"]) is float:
                controller_cfg["Kp"]["translation"] = [
                    controller_cfg["Kp"]["translation"]
            ...
            if not check_attr(controller_cfg["Kp"], "rotation"):
                controller_cfg["Kp"]["rotation"] = [1.0] * 3
                field_missing = True
            elif type(controller_cfg["Kp"]["rotation"]) is float:
                controller_cfg["Kp"]["rotation"] = [
                    controller_cfg["Kp"]["rotation"]
            ...
            if not check_attr(controller_cfg, "Kd"):
                controller_cfg["Kd"] = 0.0
            else:
                controller_cfg["Kd"] = float(controller_cfg["Kd"])

            if not check_attr(controller_cfg, "traj_interpolator_config"):
                controller_cfg["traj_interpolator_config"] = {
                    "max_velocity": 0.25,
